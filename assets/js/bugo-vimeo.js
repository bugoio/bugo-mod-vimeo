...
/* harmony import */ var _video_worker_esm__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! ./video-worker.esm */ "./src/es6/video-worker.esm.js");
...
/*#__PURE__*/
function () {
  function BugoVimeoThumb(container) {
    _classCallCheck(this, BugoVimeoThumb);
    ...
    this.player = false;
    this.video = new _video_worker_esm__WEBPACK_IMPORTED_MODULE_1__["default"](this.videoUrl, {
      autoplay: false,
      loop: false,
      startTime: 0
    }); // console.log(this);
  ...
  _createClass(BugoVimeoThumb, [{
    ...
    value: function initVideo(id) {
      ...
      if (this.video.isValid()) {
        ...
        this.video.getVideo(function (video) {
          var $parent = video.parentNode;
          console.log(_this.videoWindow); // insert video in the body.  

          $(_this.videoWindow).append(video);
          console.log($, jQuery); // remove temporary parent video element (created by VideoWorker).

          $parent.parentNode.removeChild($parent);

          _this.video.getMeta(function (meta) {
            console.log(meta);
            _this.thumbnail.src = meta.videoImage;
            $(_this.header).append(meta.title);
            $(_this.summary).append(meta.description);

            _this.button.setAttribute('aria-label', "Play " + meta.title);

            _this.player = _this.videoWindow.querySelector('iframe');

            _this.player.setAttribute('role', 'presentation');
    ...
    key: "initPlayer",
    value: function initPlayer(player) {
      var _this2 = this;

      var self = this;
      this.video.on('play', function (event) {
        _this2.isPlaying = true;
        console.log('play', _this2, player);
      });
      this.video.on('stop', function (event) {
...
var YoutubeAPIKey = "AIzaSyCi39rVv5F9bMLY4LU-ghEcp0zLwAzrQlo";
...
function () {
  ...
  _createClass(VideoWorker, [{
    ...
    key: "getMeta",
    value: function getMeta(callback) {
      var self = this;
      ...
      if (self.type === 'youtube') {
        var request = new XMLHttpRequest();
        request.open('GET', "https://www.googleapis.com/youtube/v3/videos?part=snippet&id=".concat(self.videoID, "&key=").concat(self.YoutubeAPIKey), true);

        request.onreadystatechange = function () {
          if (this.readyState === 4) {
            if (this.status >= 200 && this.status < 400) {
              // Success!
              var response = JSON.parse(this.responseText).items[0].snippet;
      ...
      if (self.type === 'vimeo') {
        ...
        _request.open('GET', "https://vimeo.com/api/v2/video/".concat(self.videoID, ".json"), true);
